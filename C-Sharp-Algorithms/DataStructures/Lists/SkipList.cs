...
namespace DataStructures.Lists
{
    ...
    public class SkipList<T> : ICollection<T>, IEnumerable<T> where T : IComparable<T>
    {
        ...
        public void Add(T item)
        {
            ...
            // Get the next node level
            int lvl = Math.Max(1, _getNextLevel());
            if (lvl > _currentMaxLevel)
            {
                for (int i = _currentMaxLevel; i < lvl; ++i)
                    toBeUpdated[i] = _firstNode;
                // _currentMaxLevel = lvl; // Move this after node insertion
            }
            ...
            // Now update the current max level if needed
            if (lvl > _currentMaxLevel)
        ...
        public bool Find(T item, out T result)
        {
            ...
            // Return true if we found the element and it's not the sentinel node; false otherwise
            if (current != _firstNode && current.Value.IsEqualTo(item))
            {
